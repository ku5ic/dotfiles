...
`claude/bin/git-diff-from-base.sh` diffs the current branch against its base, resolved by `git-base.sh` (upstream, then `origin/HEAD`, then main/master/develop/trunk) or a base passed as the first argument. Both are on `$PATH` via `claude/bin`, so they are callable by bare name.
