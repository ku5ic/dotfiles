...
- Frontend surface must exist. If the injected `<repo-context>` block reports no `js`, no `ruby` with rails, no `python` with django templates, and no HTML files: stop and say so.
