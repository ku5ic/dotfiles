...
| Validate a GitHub Actions workflow                                                   | `actionlint`                                                                                                                                        | reading and reasoning about YAML by eye        |
| Symbol/dependency lookup (who calls X, callers/callees, structural "how is Y wired") | `mcp__codebase-memory-mcp__search_graph` / `trace_path` / `get_architecture` (project must be indexed - check `list_projects`/`index_status` first) | grep + manual multi-file reads                 |

Graph tools answer structural questions in one call where grep needs a multi-file read chain; they only cover indexed projects, so fall back to `rg` for literal text search or a repo that is not indexed.
