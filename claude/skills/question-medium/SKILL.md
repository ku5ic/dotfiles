...
2. Verify before answering when the question warrants it.
   - Concerns a specific file, project, or repo: read the relevant files rather than assuming.
   - Concerns the current version, features, or API of a fast moving tool: check the lockfile or authoritative source.
3. If the stack matters:
   ...
   - Load the matching patterns skill if the question is in that area.
