...
argument-hint: <path to an existing audit report in ~/.claude/scratch/>
model: sonnet
effort: high
...
1. Resolve the report path from $ARGUMENTS. If empty or the file does not exist, list the most recent reports (`ls -t ~/.claude/scratch/*.md | head -10`) and ask which one via AskUserQuestion rather than guessing.
...
Write to `~/.claude/scratch/verify-<project-name>-<original-slug>-<YYYYMMDD-HHMM>.md`, where `<original-slug>` is the original report's own target slug, or its full kind-and-timestamp stem if it has none (e.g. `audit-claude-*` reports). Never modify the original report -- this command is append-only across files, so a bad verify run cannot corrupt an audit.
